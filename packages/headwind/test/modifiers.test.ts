  ...
  describe('Edge Cases', () => {
    describe('Important with all utility types', () => {
      it('should work with display utilities', () => {
        const gen = new CSSGenerator(defaultConfig)
        gen.generate('!block')
        expect(gen.toCSS()).toContain('display: block !important;')
      })

      it('should work with flexbox utilities', () => {
        const gen = new CSSGenerator(defaultConfig)
        gen.generate('!flex-col')
        expect(gen.toCSS()).toContain('flex-direction: column !important;')
      })

      it('should work with grid utilities', () => {
        const gen = new CSSGenerator(defaultConfig)
        gen.generate('!grid-cols-3')
        expect(gen.toCSS()).toContain('grid-template-columns: repeat(3, minmax(0, 1fr)) !important;')
      })

      it('should work with transform utilities', () => {
        const gen = new CSSGenerator(defaultConfig)
        gen.generate('!rotate-45')
        expect(gen.toCSS()).toContain('transform: rotate(45deg) !important;')
      })

      it('should work with opacity', () => {
        const gen = new CSSGenerator(defaultConfig)
        gen.generate('!opacity-50')
        expect(gen.toCSS()).toContain('opacity: 0.5 !important;')
      })

      it('should work with border utilities', () => {
        const gen = new CSSGenerator(defaultConfig)
        gen.generate('!border')
        expect(gen.toCSS()).toContain('border-width: 1px !important;')
      })

      it('should work with rounded utilities', () => {
        const gen = new CSSGenerator(defaultConfig)
        gen.generate('!rounded-lg')
        expect(gen.toCSS()).toContain('border-radius: 0.5rem !important;')
      })

      it('should work with shadow utilities', () => {
        const gen = new CSSGenerator(defaultConfig)
        gen.generate('!shadow-md')
        expect(gen.toCSS()).toContain('box-shadow:')
        expect(gen.toCSS()).toContain('!important')
      })
    })
    ...
    describe('Important with arbitrary values', () => {
      it('should work with arbitrary width', () => {
        ...
        expect(gen.toCSS()).toContain('width: 350px !important;')
      })

      it('should work with arbitrary color', () => {
        const gen = new CSSGenerator(defaultConfig)
        gen.generate('!bg-[#1a1a1a]')
        expect(gen.toCSS()).toContain('background-color: #1a1a1a !important;')
      })

      it('should work with arbitrary spacing', () => {
        const gen = new CSSGenerator(defaultConfig)
        gen.generate('!gap-[2.5rem]')
        expect(gen.toCSS()).toContain('gap: 2.5rem !important;')
      })
    })
    ...
    describe('Important edge cases', () => {
      ...
      it('should handle important with zero values', () => {
        const gen = new CSSGenerator(defaultConfig)
        gen.generate('!p-0')
        expect(gen.toCSS()).toContain('padding: 0 !important;')
      })

      it('should handle important with fractional values', () => {
        const gen = new CSSGenerator(defaultConfig)
        gen.generate('!w-1/2')
        expect(gen.toCSS()).toContain('width: 50% !important;')
      })
